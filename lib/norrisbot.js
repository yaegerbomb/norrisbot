...
NorrisBoy.prototype._replyWithKeywordJoke = function(originalMessage){
	var self = this;
	...
	if(indexOfChuckNorris == -1){
		indexOfChuckNorris = messageText.indexOf(this.name);		
		keyword = messageText.split(this.name)[1];
	}else{
		keyword = messageText.split('chuck norris');
	}
